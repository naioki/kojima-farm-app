...
PRODUCER_NAME = st.secrets.get("PRODUCER_NAME", "小島農園")
API_KEY = st.secrets.get("GEMINI_API_KEY", "")

client = genai.Client(api_key=API_KEY)

# 数字以外の文字が混じっていても数値化するヘルパー関数
def safe_int(v):
...
def get_order_data(image):
    ...
    【重要】unit, boxes, remainderには「数字のみ」を入れてください。単位は不要です。
    【ルール】胡瓜(3本P):30/箱, 胡瓜(バラ):100/箱(50以上なら50本箱1,未満バラ), 春菊:30/箱, 青梗菜:20/箱, 長ネギ(2本P):30/箱
    【出力JSON】[{"store":"店舗名","item":"品目名","spec":"規格(例:2L)","unit":"30","boxes":"5","remainder":"0"}]"""
...
def create_b5_pdf(data):
    ...
    for entry in data:
        ...
        # 8. マテハン名（空欄）
        pdf.set_font('Gothic', size=14)
        pdf.cell(col1, h, " マテハン名", border=1)
